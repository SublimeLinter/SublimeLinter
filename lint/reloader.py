...
Under MIT License (MIT) see LICENSE in top folder

Additonal copyright holders:

Copyright (c) 2015 Eldar Abusalimov, @abusalimov
Copyright (c) 2017 @deathaxe
Copyright (c) 2018 @kaste

This is a slightly modified version of GitGutters reloader, which is
a fork of GitSavvy's reloader.
"""
...
def reload_everything():
    reload_core()
    reload_linter_plugins()


def reload_core():
    reload_package('SublimeLinter.lint')
    reload_plugin('SublimeLinter.')


def reload_linter_plugins():
    reload_plugin('SublimeLinter-')
...
def reload_package(pkg_name, perform_reload=True):
    """Reload a package among with all its modules."""
...
def reload_modules(main, modules, perform_reload=True):
    ...
    - Hide modules from the sys.modules temporarily
